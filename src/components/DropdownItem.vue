...
<template>
  ...
  <div v-else class="relative group/item">
    ...
    <component
        :is="getLinkComponent(item)"
        v-bind="getLinkProps(item)"
        class="flex items-center justify-between gap-3 px-4 py-2 text-sm text-zinc-500 hover:text-zinc-500 hover:bg-zinc-50 transition-colors duration-150 cursor-pointer w-full"
    >
